...
export default (Alpine: typeof AlpineInstance) => {
  Alpine.directive('light', (el,  {expression}, {evaluateLater, effect}) => {
    ...
    (effect as any)(() => {
      const light = (el as NorskaElement).__norska.light;
      if (light) {
        ...
        getValues(([name, args, options]: Props) => {
          (el as NorskaElement).__norska.light = new lights[name](args);
          Object.assign((el as any).__norska.light, options);
          scene.add((el as NorskaElement).__norska.light);
        });
      }
    });
  });
